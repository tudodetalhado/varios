    ...
    ui(new Ui::MainWindow)
{
    ...
    layout->addWidget(sliderVermelho, 0, Qt::AlignTop);
    sliderVermelho->setMinimum(0);
    sliderVermelho->setMaximum(255);
    sliderVermelho->setPositions(50, 200);

    QLabel* lblAzul = new QLabel(tr("Azul"));
    lblAzul->setFont(fonte);
    layout->addWidget(lblAzul, 0, Qt::AlignTop);
    ...
    layout->addWidget(sliderAzul, 0, Qt::AlignTop);
    sliderAzul->setMinimum(0);
    sliderAzul->setMaximum(255);
    sliderAzul->setPositions(50, 200);

    QLabel* lblMatiz = new QLabel(tr("Matiz"));
    lblMatiz->setFont(fonte);
    layout->addWidget(lblMatiz, 0, Qt::AlignTop);
    ...
    layout->addWidget(sliderSaturar, 0, Qt::AlignTop);
    sliderSaturar->setMinimum(0);
    sliderSaturar->setMaximum(255);
    sliderSaturar->setPositions(50, 200);

    QLabel* lblValor = new QLabel(tr("Brilho"));
    lblValor->setFont(fonte);
    layout->addWidget(lblValor, 0, Qt::AlignTop);
    ...
    layout->addWidget(sliderValor, 0, Qt::AlignTop);
    sliderValor->setMinimum(0);
    sliderValor->setMaximum(255);
    sliderValor->setPositions(50, 200);

    QLabel* lblContraste = new QLabel(tr("Contraste"));
    lblContraste->setFont(fonte);
    layout->addWidget(lblContraste, 0, Qt::AlignTop);

    ctkRangeSlider* sliderContraste = new ctkRangeSlider(Qt::Horizontal, 0);
    sliderContraste->connect(sliderContraste, SIGNAL(valuesChanged(int, int)), this, SLOT(ajustarContraste(int,int)));
    layout->addWidget(sliderContraste, 0, Qt::AlignTop);
    sliderContraste->setMinimum(0);
    sliderContraste->setMaximum(255);
    sliderContraste->setPositions(50, 200);
...
//    int newValue = 0;
    ...
    //cv::GaussianBlur(frmMascara, frmDesfoco, cv::Size(25,25), 0);
    ...
    cv::erode(frmCinzaRedondo, frmCinzaRedondo, elementErode);
    ...
    cv::dilate(frmCinzaRedondo, frmCinzaRedondo, elementDilate);
    ...
    */
    ...
    if (indices.size() > 0){
       ...
       cv::drawContours(frmCor, contours, indices[0], cv::Scalar(255, 0, 0), 10);
       cv::drawContours(frmHSVContraste, contours, indices[0], cv::Scalar(150, 150, 150), 10);
...
//                cv::addWeighted(frameTemp, 0.7, frmRastros, 1, 1, frameTemp);
//            }
//            _x = centroid.x;
//            _y = centroid.y;
//        }
    }
    //TERMINA O RASTREAMENTO A PARTIR DAQUI--------------------------------

    bool mostrarRGB = ui->rdbRGB->isChecked();
    if (mostrarRGB)
    {
        // Adicionar o registro
        cv::circle(frmCor, cv::Point(_eixoX, _eixoY), _raio, vermelho, 3);
        cv::circle(frmCor, cv::Point(_eixoX, _eixoY), _raio - 35, amarelo, 1);
        //cv::circle(frameColor, cv::Point((_eixoX - (_raio / 3)), (_eixoY - (_raio / 3))),  _raio / 9, amarelo, 2);
        cv::circle(frmCor, cv::Point(((_eixoX - (_raio / 3)) + 18), ((_eixoY - (_raio / 3))- 20)), _raio / 9, amarelo, 1);
        cv::line(frmCor, cv::Point(_eixoX, (_eixoY - _raio - 20)), cv::Point(_eixoX, (_eixoY + _raio + 20)), amarelo, 1, cv::LINE_8);
        cv::line(frmCor, cv::Point((_eixoX - _raio - 20), _eixoY), cv::Point((_eixoX + _raio + 20), _eixoY), amarelo, 1, cv::LINE_8);

        printType(frmCor);
        QImage image = QImage(frmCor.data, 512, 424, QImage::Format_RGB32);
        ui->lblViewResultadoPlay->setPixmap(QPixmap::fromImage(image));
    }

    bool mostrarMascara = ui->rdbHSV->isChecked();
    if (mostrarMascara)
    {
        cv::cvtColor(frmMascara, frmMascaraFinal, CV_GRAY2BGR);

        // Adicionar o registro
        cv::circle(frmMascaraFinal, cv::Point(_eixoX, _eixoY), _raio, vermelho, 3);
        cv::circle(frmMascaraFinal, cv::Point(_eixoX, _eixoY), _raio - 35, amarelo, 1);
        //cv::circle(frameColor, cv::Point((_eixoX - (_raio / 3)), (_eixoY - (_raio / 3))),  _raio / 9, amarelo, 2);
        cv::circle(frmMascaraFinal, cv::Point(((_eixoX - (_raio / 3)) + 18), ((_eixoY - (_raio / 3))- 20)), _raio / 9, amarelo, 1);
        cv::line(frmMascaraFinal, cv::Point(_eixoX, (_eixoY - _raio - 20)), cv::Point(_eixoX, (_eixoY + _raio + 20)), amarelo, 1, cv::LINE_8);
        cv::line(frmMascaraFinal, cv::Point((_eixoX - _raio - 20), _eixoY), cv::Point((_eixoX + _raio + 20), _eixoY), amarelo, 1, cv::LINE_8);
    ...
    bool mostrarContraste = ui->rdbConstraste->isChecked();
    if (mostrarContraste)
    {
        cv::cvtColor(frmContraste, frmContrasteFinal, CV_GRAY2BGR);

        // Adicionar o registro
        cv::circle(frmContrasteFinal, cv::Point(_eixoX, _eixoY), _raio, vermelho, 3);
        cv::circle(frmContrasteFinal, cv::Point(_eixoX, _eixoY), _raio - 35, amarelo, 1);
        //cv::circle(frameColor, cv::Point((_eixoX - (_raio / 3)), (_eixoY - (_raio / 3))),  _raio / 9, amarelo, 2);
        cv::circle(frmContrasteFinal, cv::Point(((_eixoX - (_raio / 3)) + 18), ((_eixoY - (_raio / 3))- 20)), _raio / 9, amarelo, 1);
        cv::line(frmContrasteFinal, cv::Point(_eixoX, (_eixoY - _raio - 20)), cv::Point(_eixoX, (_eixoY + _raio + 20)), amarelo, 1, cv::LINE_8);
        cv::line(frmContrasteFinal, cv::Point((_eixoX - _raio - 20), _eixoY), cv::Point((_eixoX + _raio + 20), _eixoY), amarelo, 1, cv::LINE_8);
    ...
    bool mostrarInfravermelho = ui->rdbInfravermelho->isChecked();
    if (mostrarInfravermelho)
    {
        //cv::cvtColor(frmHSVContraste, frmHSVFinal, CV_GRAY2BGR);

        // Adicionar o registro
        cv::circle(frmHSVFinal, cv::Point(_eixoX, _eixoY), _raio, vermelho, 3);
        cv::circle(frmHSVFinal, cv::Point(_eixoX, _eixoY), _raio - 35, amarelo, 1);
        //cv::circle(frameColor, cv::Point((_eixoX - (_raio / 3)), (_eixoY - (_raio / 3))),  _raio / 9, amarelo, 2);
        cv::circle(frmHSVFinal, cv::Point(((_eixoX - (_raio / 3)) + 18), ((_eixoY - (_raio / 3))- 20)), _raio / 9, amarelo, 1);
        cv::line(frmHSVFinal, cv::Point(_eixoX, (_eixoY - _raio - 20)), cv::Point(_eixoX, (_eixoY + _raio + 20)), amarelo, 1, cv::LINE_8);
        cv::line(frmHSVFinal, cv::Point((_eixoX - _raio - 20), _eixoY), cv::Point((_eixoX + _raio + 20), _eixoY), amarelo, 1, cv::LINE_8);
