...
void KinectWriter::openNewFileForWriting(wchar_t* ext)
{
	closeFileIfOpened();
    ...
    sprintf(filename, "captura_%04d_%02d_%02d_%02d_%02d.%ls",
            now->tm_year + 1900,
            now->tm_mon + 1,
            now->tm_mday,
            now->tm_hour,
...
void KinectWriter::writeColorFrame(RGBQUAD* colorBuffer, UINT size)
{
