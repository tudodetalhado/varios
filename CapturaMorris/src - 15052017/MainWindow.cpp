...
//    usedSlots = new QSemaphore(0);
    ...
    //cor
    _frm_1920x1080_cor = new cv::Mat(cv::Mat::zeros(1080, 1920, CV_8UC4));
...
//    sharedImageBuffer->setSyncEnabled(true);
    ...
    int milliseconds = 22;
    sensorKinect = new KinectSensor(_deveGravarVideo, milliseconds);
    connect(sensorKinect, SIGNAL(kinectColorCallback(byte*)), this, SLOT(onKinectCallback(byte*)));
    connect(ui->chkGravar, SIGNAL(pararDeGravar()), sensorKinect, SLOT(pararDeGravar()));
...
void MainWindow::onKinectCallback(byte* buffer)
{
    //FPS(ui->lblFPS, time->valor);

    //_qImage = QImage((uchar*)buffer, 640, 530, QImage::Format_ARGB32);
    _qImage = QImage((uchar*)buffer, 1920, 1080, QImage::Format_ARGB32);
    _cena->addPixmap(QPixmap::fromImage(_qImage));
    ui->viewCaptura->setCena(_cena);

    //ui->label->setPixmap(QPixmap::fromImage(QImage((uchar*) buffer,
    //  cColorWidth, cColorHeight, QImage::Format_RGB32).scaledToWidth(1280)));


    //QByteArray byteArray = QByteArray(_bufferColorKinect);
    //if (_bufferColorKinect != NULL)
    //if(_bufferColorKinect != 0)
    //{
        //Mat frm = Mat(1080, 1920, CV_8UC4, reinterpret_cast<void*>(_bufferColorKinect));
        //uchar* data = reinterpret_cast<uchar*>(_bufferColorKinect);
    ...
    //{
        ...
        //QPixmap qPixmap = QPixmap::fromImage(QImage((uchar*) _1920x1080_cor_buffer, _frmWidth_1920, _frmHeight_1080,
        //QImage::Format_RGB32).scaledToHeight(ui->viewCaptura->frameSize().height()));
          //QImage::Format_RGB32).scaled(ui->viewCaptura->frameSize()));

        //cv::resize(_frm_1920x1080_cor, _frm_1920x1080_cor, cv::Size(_frmWidth_1920, _frmHeight_1080));
        //_frm_1920x1080_cor = cv::resize();


//        _qImage = QImage((uchar*) _frm_1920x1080_cor->data,
//              _frmWidth_1920, _frmHeight_1080, QImage::Format_RGB888);
//        _cena->addPixmap(QPixmap::fromImage(_qImage));
//                ui->viewCaptura->setCena(_cena);

             //_frmWidth_1920, _frmHeight_1080, QImage::Format_RGB32);
        //int len = 1920 * 1080 * sizeof(RGBQUAD);
        //_qImage.loadFromData((uchar*) _1920x1080_cor_buffer, len);
...
void MainWindow::posicionarY(int valor)
{
    ...
    ui->sliderCirculoY->setMaximum(540 - _raioArena);

    if(valor >= _raioArena && valor <= (540 - _raioArena))// (_raio * 2))
    {
       _eixoYArena = valor;
    }
}
