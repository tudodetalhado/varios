...
KinectSensor::KinectSensor(bool deveGravarVideo, int milliseconds) : QThread()
{
    _deveGravar = deveGravarVideo;


    //_bufferColorKinect = _640x530_cor_buffer;
    //_640x530_cor_buffer = corBuffer;
    ...
    //cor
    _1920x1080_cor_buffer = new RGBQUAD[_frmColorWidth * _frmColorHeight];
    _1304x1080_cor_buffer = new RGBQUAD[1304 * 1080];
    _640x530_cor_buffer   = new RGBQUAD[640  *  530];

    _bufferColorKinect = new byte[1920 * 1080 * 4];

    _colorBufferSize = UINT(1920 * 1080 * sizeof(RGBQUAD));
...
//        doStopMutex.unlock();
        ...
        if (capturarFrame())
        {
            emit kinectColorCallback(_bufferColorKinect);

//            if (_deveGravar)
//            {
//                cv::Mat frmCor(530, 640, CV_8UC4, _640x530_cor_buffer);
//                _videoWriter << frmCor;
//                frmCor.release();
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                //_bufferColorKinect = nullptr;

                //_bufferColorKinect = new BYTE[colorBufferSize];
                //hr = colorFrm->CopyConvertedFrameDataToArray(static_cast<UINT>(_colorBufferSize), reinterpret_cast<BYTE*>(_1920x1080_cor_buffer), ColorImageFormat_Bgra);
                //hr = colorFrm->CopyConvertedFrameDataToArray(colorBufferSize, _bufferColorKinect, ColorImageFormat_Bgra);
                //hr = colorFrm->AccessRawUnderlyingBuffer(&colorBufferSize, &_bufferColorKinect);
                //if (SUCCEEDED(hr))
                //{
                //    capturou = true;
                //}
                ...
                //const int imgSize = frameWidth*frameHeight * 4;
                UINT colorBufferSize = 1920 * 1080 * 4;
                //ColorImageFormat imageFormat = ColorImageFormat_None;
//                hr = colorFrm->get_RelativeTime(&_colorTime);
//                hr = colorFrm->get_RawColorImageFormat(&ColorImageFormat_None);

//                UINT size = _frmColorWidth * _frmColorHeight * 4;// static_cast<UINT>(_frm_1920x1080_cor->elemSize());
//                colorFrm->CopyConvertedFrameDataToArray(size, _frm_1920x1080_cor->data, ColorImageFormat_Bgra);
...
void KinectSensor::pararDeGravar()
{
