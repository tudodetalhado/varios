...
private:
    ...
    unsigned int depthBytesPerPixel = 2;
    ...
    void alignColor2Depth(const UINT16* pDepthBuffer, const RGBQUAD* pColorBuffer);

    void ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight);
    void ProcessColorCropped(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight);
};
