...
int CColorBasics::Run(HINSTANCE hInstance, int nCmdShow)
{
    ...
    wc.hIcon         = LoadIconA(hInstance, MAKEINTRESOURCE(IDI_APP));
    ...
    HWND hWndApp = CreateDialogParamA(
        NULL,
...
void CColorBasics::ProcessColor(INT64 nTime, RGBQUAD* pBuffer, int nWidth, int nHeight) 
{
    if (m_hWnd)
    {
        ...
        //StringCchPrintf(szStatusMessage, _countof(szStatusMessage), " FPS = %0.2f    Time = %I64d", fps, (nTime - m_nStartTime));
    ...
    if (pBuffer && (nWidth == cColorWidth) && (nHeight == cColorHeight))
    {
        ...
        if (m_bSaveScreenshot)
        {
            ...
            if (SUCCEEDED(hr))
            {
                ...
                //StringCchPrintf(szStatusMessage, _countof(szStatusMessage), "Screenshot saved to %s", szScreenshotPath);
            }
            else
            {
                //StringCchPrintf(szStatusMessage, _countof(szStatusMessage), "Failed to write screenshot to %s", szScreenshotPath);
            }
